...
def show():
    ...
    sales_data = get_sales(period_api)
    mkt_data   = get_marketing(period_api)
    if not sales_data:
    ...
    top_services = sales_data.get("top_services", [])
    ...
    # Revenue split only shown when company has both
    ...
    # ── Revenue by channel + Top products (side by side) ─────────────────────
    ...
    with c_right:
        ...
        if top_products:
            df_prod = pd.DataFrame(top_products).set_index("product_name")
